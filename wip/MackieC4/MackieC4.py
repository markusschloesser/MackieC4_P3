...
class MackieC4(object):
    ...
    For example, instead of ramping up to full acceleration over fifteen quick little encoder knob clicks
    I think that data could be scaled out so it takes maybe 30 little clicks before full throttle
    Probably need to experiment with the size of the interpolated acceleration range
    
    # We would be scaling a C4 value from 1 - 15 over 30 steps    
    last_index = len(encoder_cw_values) - 1 
    jog_wheel_scaler = make_interpolater(encoder_cw_values[0], encoder_cw_values[last_index], 1, 30)
    def handle_jog_wheel_rotation(self, cc_value):
        if cc_value in encoder_ccw_values: # counter-clock-wise
            self.song().jump_by(-jog_wheel_scaler(cc_value - 64))  # (cc_value - 0x40)
        else:
            self.song().jump_by(jog_wheel_scaler(cc_value))
    """
    def handle_jog_wheel_rotation(self, cc_value):
