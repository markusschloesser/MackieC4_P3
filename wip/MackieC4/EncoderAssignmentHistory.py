...
class SongData(object):
    ...
    def __init__(self, logger=None, get_device_list=None):
        ...
        self.device_list_table = dict[str, dict[int, ActiveDeviceList]]({
    ...
    def get_active_device_list_reference(self, track_callback_type_key, track_index_by_type) -> ActiveDeviceList | None:
    ...
    def get_all_tracks_by_type_key(self, track_callback_type_key) -> Dict[int, ActiveDeviceList]:
    ...
    def remove_device_by_track_callback_type(self, track_callback_type_key, callback_type_index, device_index_before_remove_index):
        ...
        self._collapse_track_device_list_slot(device_map, device_index_before_remove_index)
    ...
    def _insert_track_slot(self, type_dict: Dict[int, ActiveDeviceList], insert_at_track_callback_type_index, track_device_list_ref: ActiveDeviceList)-> dict[int, ActiveDeviceList]:
    ...
    def _insert_track_device_list_slot(self, type_dict: Dict[int, ActiveDevice], insert_at_device_index, active_device: ActiveDevice)-> dict[int, ActiveDevice]:
    ...
    def _collapse_track_device_list_slot(self, type_dict, device_index_left_of_collapse_index)-> dict[int, ActiveDevice]:
        log_id = "EAH.SD._collapse_track_device_list_slot: "
    ...
    def __shift_keys_right(self, local_dict: dict[int, ActiveDevice]|dict[int,ActiveDeviceList], key_of_add, value_to_add: ActiveDevice|ActiveDeviceList) -> dict[int, ActiveDevice]|dict[int,ActiveDeviceList]:
    ...
    def __shift_keys_left(self, local_dict: Dict[int, ActiveDevice]|Dict[int,ActiveDeviceList], key_before_del) -> Dict[int, ActiveDevice]|Dict[int,ActiveDeviceList]:
...
class EncoderAssignmentHistory(MackieC4Component):
    ...
    def tracks_deleted(self, selected_song_track_index_after, tracks_of_type_after, callback_type):
        ...
        if callback_type == 0:
            ...
            # at_index = selected_song_track_index_after
            ...
            # at_rtns_index = selected_song_track_index_after - self.data.plain_track_count
    ...
    def unselected_tracks_deleted(self, found_changed_track_callback_type, callback_type_track_count):
        ...
        while len(changed_track_type_table.keys()) > callback_type_track_count and at_index < table_size:
            ...
            if liveobj_valid(track_ref):
                ...
                # execution might also land here if 2 or more unselected tracks were (not just removed from view but) deleted at the same time (not possible?)
    ...
    def update_device_counts_on_change(self, all_track_devices, selected_device, old_selected_device_index, changed_device_index, new_device_count_track):
        ...
        last_track_ref = self.data.get_track(self.last_selected_track_index)
        # from_none = True if old_selected_device_index is None else False
        # to_none = True if changed_device_index is None else False
        # from_index = old_selected_device_index if from_none else int(old_selected_device_index)
        # to_index = changed_device_index if to_none else int(changed_device_index)

        # last_selected_device_ref = self.data.get_device(self.last_selected_track_index, self.last_selected_device_index)
        # if (from_index != to_index and last_selected_device_ref is not None and liveobj_valid(last_selected_device_ref.device) and
        #         not liveobj_changed(last_selected_device_ref.device, selected_device)):
        #     msg = f"{log_id}selected device remains {last_selected_device_ref.device_name} but selected device index of track {last_track_ref.track_name} changed from "
        #     self.main_script().log_message(logging.DEBUG, msg + f"{old_selected_device_index} to {changed_device_index}, moving device reference to changed index")
