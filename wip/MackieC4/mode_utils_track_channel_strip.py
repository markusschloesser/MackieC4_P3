...
def do_display_update(app_view, selected_track, chosen_plugin, is_locked_to_device, display_parameters,
    ...
    replace_char = '+' if not is_expanding_chains else '|'
    for i in range(len(lcd1_device_chain_flags) - 1):
        if lcd1_device_chain_flags[i]: # flag indexes 0 - 6
            ...
            # there are probably more Pythonic ways to replace chars than slicing, but this line of code is easy to read
            upper_string2 = upper_string2[:replace_index] + replace_char + upper_string2[replace_index:]
    # now if for example, the devices associated with encoders 10, 12, 13, 15 have a class_name that ends with "GroupDevice", and chains are expanded,
