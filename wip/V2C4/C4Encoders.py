...
class C4Encoders:
    ...
    def build_midi_map(self, midi_map_handle):
        if self.__parent.is_enabled() and self.__parent.track is not None:
            track = self.__parent.track
                ...
                if assignment_necessary:
                    ...
                    for encoder_index in range(ENCODER_BASE, NUM_ENCODERS):
                        ...
                        if len(device_parameters) + free_encoders >= parameter_index:
                            self.specialize_feedback_rule(feedback_rule, c4_cc_channel)
                            parameter = 0
                            ...
                            if parameter:
                                Live.MidiMap.map_midi_cc_with_feedback_map(midi_map_handle, parameter, c4_cc_channel,
